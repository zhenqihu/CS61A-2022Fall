def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + lists for lists in nested_list]
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        """
        A function return a list of lists,
        containing all the in-order, non-decreasing subsequence of s,
        in which all the elements are greater or equal to `prev`
        """
        if not s:
            return [[]]
        elif s[0] < prev:
            ...
            # all non-decreasing subsequence of s[1:] in which all elements >= s[0]
            # e.g. [[], [2], [3]] in case 1
            # e.g. [[], [1], [1, 2], [2]] in case 2
            b = subseq_helper(s[1:], prev)
            # all non-decreasing subsequence of s[1:] in which all elements >= prev (0)
            # e.g. [[], [2], [3]] in case 1
            # e.g. [[], [1], [1, 2], [2]] in case 2
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            return 'Please update your balance with $' + str(diff) + ' more funds.'
        elif self.balance > self.price:
            diff = self.balance - self.price
            self.balance = 0
            self.stock -= 1
            return 'Here is your ' + self.product + ' and $' + str(diff) + ' change.'
        else:
            self.balance = 0
            self.stock -= 1
...
def long_paths(t, n):
    ...
    lst = []
    for b in t.branches:
        lst.extend(long_paths(b, n - 1))
    return [[t.label] + path for path in lst]
...
def reverse_other(t):
    ...
    def helper(tree, dep):
        if tree.is_leaf():
            ...
            label = [b.label for b in tree.branches]
            label_reverse = label[::-1]
            for i in range(len(tree.branches)):
                tree.branches[i].label = label_reverse[i]
